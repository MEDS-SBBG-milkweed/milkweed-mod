...
We'll want to mask the model raster template to the northern and southern regions of the LPNF before we run any distance calculations. We'll use the RasterLayer (`terra`) for this and the `raster::mask()` function.
