...
Next, we'll crop all of our layers other than Land Ownership to the LPNF boundary by using the polygons that represent the northern and southern regions as masks. We are doing this before rescaling because rescaling before cropping may result in highest or lowest values being cropped out of our areas of interest, which impacts the final rescaled layers as well as the final survey site accessibility index. We'll mask Land Ownership after we rescale it because it will only have values of 0 and 1, with no threat of cutting maximum or minimum values out when we mask it afterwards.
...
ownership_masked <- terra::mask(ownership_rast, lpnf_boundary)
plot(ownership_masked)
```
