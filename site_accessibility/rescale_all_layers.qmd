...
## Canopy Cover
```{r}
canopy_rescaled <- rescale_raster(canopy_reproject)
...
## Slope
```{r}
slope_rescaled <- rescale_raster(slope_reproject_avg)
...
```{r}
canopy_final <- 1- (mask(canopy_rescaled, lpnf_boundary_buffered))

slope_final <- 1 - (mask(slope_rescaled, lpnf_boundary_buffered))
...
## save the slope (resampled by average) rescaled
```{r}
# writeRaster(slope_final, here("~/../../capstone/milkweedmod/outputs/site_accessibility_outputs/slope_avg_rescaled.tif"))
