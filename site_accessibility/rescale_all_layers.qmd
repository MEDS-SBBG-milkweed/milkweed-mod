...
## Canopy Cover
```{r}
canopy_rescaled <- rescale_raster(canopy_cover)
...
## Slope
```{r}
slope_rescaled <- rescale_raster(slope)
...
```{r}
trails_rescaled <- rescale_raster(trails)
...
## Distance from Roads
```{r}
roads_rescaled <- rescale_raster(roads)
