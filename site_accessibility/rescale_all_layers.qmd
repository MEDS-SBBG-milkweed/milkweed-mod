...
```{r}
min_max_trails <- minmax(trails) 
# (value - minimum_vale) / (maximum_value - minimum_value)
trails_rescaled <- (trails - min_max_trails[1,]) / (min_max_trails[2,] - min_max_trails[1,])
...
## Distance from Roads
```{r}
min_max_roads <- minmax(roads) 
# (value - minimum_vale) / (maximum_value - minimum_value)
roads_rescaled <- (roads - min_max_roads[1,]) / (min_max_roads[2,] - min_max_roads[1,])
plot(roads_rescaled)
```
...
st_crs(canopy_rescaled) == st_crs(temp_rast)

# slope
print("Does slope_rescaled have correct CRS?")
st_crs(slope_rescaled) == st_crs(temp_rast)

# distance from trails
print("Does trails_rescaled have correct CRS?")
st_crs(trails_rescaled) == st_crs(temp_rast)

# distance from roads
print("Does roads_rescaled have correct CRS?")
st_crs(roads_rescaled) == st_crs(temp_rast)

# land ownership
print("Does ownership_rast have correct CRS?")
st_crs(ownership_rast) == st_crs(temp_rast)
```
...
res(canopy_rescaled) == res(temp_rast)

# slope
print("Does slope_rescaled have the correct resolution?")
res(slope_rescaled) == res(temp_rast)

# distance from trails
print("Does trails_rescaled have the correct resolution?")
res(trails_rescaled) == res(temp_rast)

# distance from roads
print("Does roads_rescaled have the correct resolution?")
res(roads_rescaled) == res(temp_rast)

# land ownership
print("Does ownership_rast have the correct resolution?")
res(ownership_rast) == res(temp_rast)
```
...
ext(canopy_rescaled) == ext(temp_rast)

# slope 
print("Does slope_rescaled have the correct extent?")
ext(slope_rescaled) == ext(temp_rast)

# distance from trails 
print("Does trails_rescaled have the correct extent?")
ext(trails_rescaled) == ext(temp_rast)

# distance from roads 
print("Does roads_rescaled have the correct extent?")
ext(roads_rescaled) == ext(temp_rast)

# land ownership 
print("Does ownership_rast have the correct extent?")
ext(ownership_rast) == ext(temp_rast)
```
## Update with template raster
```{r}
canopy_reproject <- project(canopy_rescaled,temp_rast)

# going forward (4/23) trying mode
slope_reproject_max <- project(slope_rescaled, temp_rast, method = "max")
slope_reproject_mode <- project(slope_rescaled, temp_rast, method = "mode")
plot(slope_reproject_max)
plot(slope_reproject_mode)
```

## Final Checks
```{r}
origin(ownership_rast) == origin(temp_rast)

origin(canopy_reproject)==origin(temp_rast)

origin(slope_reproject_mode)==origin(temp_rast)
...
```{r}
canopy_final <- 1- (mask(canopy_reproject, lpnf_boundary_buffered))

slope_final <- 1 - (mask(slope_reproject_mode, lpnf_boundary_buffered))

trails_final <- 1 - (mask(trails_rescaled, lpnf_boundary_buffered))

roads_final <- 1 - (mask(roads_rescaled, lpnf_boundary_buffered))
```


# Save everything in milkweedmod/ouputs/site_accessibility_outputs/

```{r}
# writeRaster(canopy_final, here("~/../../capstone/milkweedmod/outputs/site_accessibility_outputs/canopy_rescaled.tif"))
# writeRaster(slope_final, here("~/../../capstone/milkweedmod/outputs/site_accessibility_outputs/slope_rescaled.tif"))
# writeRaster(trails_final, here("~/../../capstone/milkweedmod/outputs/site_accessibility_outputs/trails_rescaled.tif"))
# writeRaster(roads_final, here("~/../../capstone/milkweedmod/outputs/site_accessibility_outputs/roads_rescaled.tif"))
# writeRaster(ownership_rast, here("~/../../capstone/milkweedmod/outputs/site_accessibility_outputs/ownership_rescaled.tif"))
```
