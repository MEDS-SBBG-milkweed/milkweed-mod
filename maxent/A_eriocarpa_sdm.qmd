---
title: "Eriocarpa: Bioclim, canopy, northness, eastness"
format: html
---

This notebook contains code for running a MaxEnt SDM for *A. eriocarpa* within the Los Padres National Forest boundary, model selection, model transfer, and mapping. 
...
```{r}
eriocarpa_model <- merge(xfer_area_Ac$xferArea, predSel_Ac) %>%
  rast()
names(eriocarpa_model) <- "fc.LQ_rm.0.5"
# writeRaster(eriocarpa_model, here("~/../../capstone/milkweedmod/outputs/sdm_outputs/eriocarpa_sdm.tif"), overwrite=TRUE)
# write_rds(model_Ac, here("~/../../capstone/milkweedmod/outputs/sdm_outputs/eriocarpa_sdm.rda"))
# writeRaster(eriocarpa_model, "outputs/dashboard/eriocarpa_sdm.tif")
```

```{r}
eriocarpa_pred <- rast(here("~/../../capstone/milkweedmod/outputs/sdm_outputs/eriocarpa_sdm.tif"))
pal_habitat <- c("#FFFFFF", "#EFCCCC", "#DF9999", "#D06666", "#C03333", "#B00000")
mapPredVals_eriocarpa <- getRasterVals(eriocarpa_pred) # change for different types
...
leaflet(options = leafletOptions(zoomControl = FALSE)) %>% 
 ...
 ##add transfer polygon (user drawn area)
  ...
  ##Add model prediction
  addRasterImage(eriocarpa_pred, colors = rasPal_eriocarpa, opacity = 0.7,
                 group = 'vis', layerId = 'mapPred', method = "ngb") %>%
  addPolygons(data = lpnf_south,
              fill = FALSE,
              color = "black",
