---
title: "Californica points bioclim canpoy"
format: html
---
...
bioclim_crop <- crop(bioclim, lpnf_south)
canopy_crop <- crop(canopy, lpnf_south)
...
# stack
envs_Ac <- raster::stack(bioclim_crop_resample, canopy_crop)
...
# bgExt_Ac <- penvs_userBgExtent(bgShp_path = "~/../../capstone/milkweedmod/data/lpnf_boundary_data/lpnf_boundary_south/lpnf_boundary_south.csv",
#                                bgShp_name = "lpnf_boundary_south.csv",
#                                userBgBuf = 0, occs = occs_Ac)
# 
# bgExt_Ac <- penvs_userBgExtent(bgShp_path = "~/../../capstone/milkweedmod/data/lpnf_boundary_data/lpnf_boundary_south/lpnf_boundary_south.shp",
#                                bgShp_name = "lpnf_boundary_south.shp", "lpnf_boundary_south.shx","lpnf_boundary_south.dbf",
#                                userBgBuf = 0, occs = occs_Ac)

multi_lpnf <- st_cast(lpnf_south, "MULTIPOINT")
south_bound_points <- multi_lpnf  %>% st_cast("POINT") %>%
  st_coordinates() %>%
  data.frame() %>%
...
```{r}
# ev <- evaluate(model_Ac@models$fc.L_rm.1, p = occs_Ac, a = bgExt_Ac, x = envs_Ac)
# plot(ev, "ROC")
```
...
m  %>%
  ...
  addLegend_decreasing("bottomleft", pal = legendPal, values = mapPredVals_Ac, layerId = "train",
            labFormat = reverseLabel(), decreasing = TRUE,
            title = "Asclepias californica<br>Predicted Suitability<br>(Training)") %>%
  addPolygons(data = lpnf_south,
              fill = FALSE,
              color = "black",
              weight = 2)
