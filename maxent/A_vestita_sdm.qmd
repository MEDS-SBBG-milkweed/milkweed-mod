...
```{r}
# merge pred area with model transfer
...
writeRaster(vestita_model, here("outputs", "dashboard", "vestita_sdm.tif"))
...
write_rds(model_Ac, here(base_dir, "outputs", "sdm_outputs", "vestita_sdm.rda"))
```

Plotting
```{r}
vestita_pred <- rast(here("outputs", "dashboard", "vestita_sdm.tif")) %>%
  mask(lpnf_boundary)
pal_habitat <- c("#FFFFFF", "#EFCCCC", "#DF9999", "#D06666", "#C03333", "#B00000")
mapPredVals_vestita <- getRasterVals(vestita_pred)
