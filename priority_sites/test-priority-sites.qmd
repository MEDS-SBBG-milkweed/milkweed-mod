...
```{r plot-test-masked}
# read in forest boundary for plotting
...
legendPal <- colorNumeric(rasCols_Ac, testVals_Ac, na.color = 'transparent', reverse = TRUE)
rasPal_Ac <- colorNumeric(rasCols_Ac, testVals_Ac, na.color = 'transparent', reverse = FALSE)
...
leaflet() %>% 
  addProviderTiles(provider = providers$Esri.WorldTerrain) %>% 
   addPolygons(data = lpnf_boundary,
              ...
              color = "black",
              weight = 3) %>% 
  addRasterImage(x = test_masked, colors = rasPal_Ac) %>% 
  addLegend_decreasing("bottomleft", pal = legendPal,
                       values = testVals_Ac, decreasing = FALSE, 
            title = "Asclepias sp. <br>Site Priority Index")
