...
```{r}
# resample to get the resolution to match bioclim (0.008333333 x 0.008333333)
resampled_raster <- resample(r, new_resolution)
```
...
```{r}
## code for dem ##
plot(terra::terrain(elevation, opt="slope"))

#### determine area of interest (with sf) ####
area_sf <- sf::st_bbox(n35w119) %>% sf::st_as_sfc() %>% sf::st_buffer(1000)
area_sp <- area_sf %>% sf::as_Spatial()

#### get digital elevation data (with elevatr) ####
dem <- elevatr::get_elev_raster(locations = elevation, prj = sf::st_crs(4326), z = 7, clip = "bbox")
