...
lpnf_bbox <- st_bbox(lpnf_reprojected)

lpnf_xmin <- lpnf_bbox[1]
lpnf_xmax <- lpnf_bbox[3]
lpnf_ymin <- lpnf_bbox[2]
lpnf_ymax <- lpnf_bbox[4]
...
Read in data
```{r}
trails_raw <- st_read(here("~/../../capstone/milkweedmod/data/2023_Regional_Trails_and_Roads_lines/2023_Regional_Trails_and_Roads_lines.shp"))

# reproject trails data
trails_reprojected <- trails |> st_transform(crs = st_crs(milkweed_reprojected))
```
...
Read in data
```{r}
soils_raw_a <- st_read(here("~/../../capstone/milkweedmod/data/soil_data/CA772/spatial/soilmu_a_ca772.shp"))
...
```{r}
all_centroids_test <- st_read("~/../../capstone/milkweedmod/data/milkweed_cleaned_data/points/all_species_points.shp")
